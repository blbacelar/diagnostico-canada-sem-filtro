...
export async function POST(request: Request) {
  try {
    ...
    if (!client) {
      const { data, error } = await admin.from("diagnostic_clients").insert({ full_name: payload.fullName, email_normalized: payload.email, email_display: payload.email, source: "hotmart" }).select("id, full_name, email_normalized, email_display").single();
