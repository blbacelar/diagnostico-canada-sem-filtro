...
export type CaseDetailData = {
  case: { id: string; case_number: string; status: string; objective: string | null; submitted_at: string | null; updated_at: string; assigned_consultant_id: string | null };
  client: { full_name: string; email_display: string };
  answers: FormAnswers;
  assessment: { id: string; version: number; structured_result: AiAssessment; created_at: string } | null;
...
export async function detailFetch<T = any>(path: string, init?: RequestInit): Promise<T> {
  const { data } = await getBrowserSupabase().auth.getSession();
  const response = await fetch(path, { ...init, headers: { ...(init?.headers ?? {}), Authorization: `Bearer ${data.session?.access_token ?? ""}` } });
  const result = await response.json() as T & { error?: string }; if (!response.ok) throw new Error(result.error ?? "Falha na operação."); return result;
}
  ...
  return <>
    ...
    <div className="detail-layout">
      <main className="detail-content">
        ...
        {ai && <>
          <section className="analysis-grid"><AnalysisCard title="Pontos fortes" icon={<Sparkles />} items={ai.strengths} tone="success" /><AnalysisCard title="Riscos principais" icon={<ShieldAlert />} items={ai.risks} tone="warning" /></section>
          {ai.technicalAlerts.length > 0 && <section className="technical-alerts"><header><AlertTriangle /><div><p className="eyebrow">Atenção técnica</p><h2>Validação profissional necessária</h2></div></header><ul>{ai.technicalAlerts.map((item) => <li key={item}>{item}</li>)}</ul><p>Alertas não determinam elegibilidade e não devem ser convertidos automaticamente em conclusões.</p></section>}
          <section className="priority-timeline"><p className="eyebrow"><Clock3 /> Plano de preparação</p><h2>Prioridades por horizonte</h2><div><PriorityColumn label="3 meses" items={ai.priorities.threeMonths} /><PriorityColumn label="6 meses" items={ai.priorities.sixMonths} /><PriorityColumn label="12 meses" items={ai.priorities.twelveMonths} /></div></section>
          <section className="analysis-grid"><AnalysisCard title="Compatibilidade regional" icon={<MapPin />} items={ai.regionalCompatibility} /><AnalysisCard title="Estimativa financeira" icon={<CircleDollarSign />} items={[ai.initialInvestmentRange, ai.recommendedReserve]} /></section>
        </>}
      ...
      <aside className="detail-sidebar"><section><p className="eyebrow">Estado do caso</p><span className={`status-pill status-${data.case.status}`}>{data.case.status.replaceAll("_", " ")}</span><dl><div><dt><CalendarDays /> Enviado</dt><dd>{data.case.submitted_at ? new Date(data.case.submitted_at).toLocaleDateString("pt-BR") : "Ainda não"}</dd></div><div><dt><Clock3 /> Atualizado</dt><dd>{new Date(data.case.updated_at).toLocaleString("pt-BR")}</dd></div><div><dt><FileText /> Parecer</dt><dd>{data.review ? `v${data.review.version} · ${data.review.status}` : "Não iniciado"}</dd></div></dl></section><section className="quick-links"><p className="eyebrow">Ações</p><Link href={`/dashboard/diagnosticos/${caseId}/relatorio`}>Pré-visualizar relatório <ArrowUpRight /></Link><Link href={`/dashboard/diagnosticos/${caseId}/email`}>Preparar e-mail <ArrowUpRight /></Link></section><section className="timeline"><p className="eyebrow">Linha do tempo</p>{data.history.map((event) => <div key={event.id}><span /><strong>{event.to_status.replaceAll("_", " ")}</strong><small>{new Date(event.created_at).toLocaleString("pt-BR")}</small></div>)}</section></aside>
    </div>
  </>;
}

function AnalysisCard({ title, icon, items, tone = "" }: { title: string; icon: React.ReactNode; items: string[]; tone?: string }) { return <article className={`analysis-card ${tone}`}><header>{icon}<h2>{title}</h2></header><ul>{items.length ? items.map((item) => <li key={item}>{item}</li>) : <li>Nenhum item registrado.</li>}</ul></article>; }
function PriorityColumn({ label, items }: { label: string; items: string[] }) { return <article><b>{label}</b><ol>{items.map((item,index) => <li key={item}><span>{index+1}</span>{item}</li>)}</ol></article>; }
